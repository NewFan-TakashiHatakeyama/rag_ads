...
- **pageId**: `assistantMessage.messageId`(英数・-・_、8〜64字)を許容(決定C)。
...
| dev 設定状態 | **段階0(enabled=false)** で待機。サンプル配信広告2件+ベクトル+紐づけ記事を保持し、`enabled=true` で即スモーク可能 |
...
## 8. Gemini統一(決定A-1)移行 Runbook

広告システムを媒体のGemini 3072次元空間へ揃える手順。**Gemini APIキーが必要**。

1. **キー共有**: 媒体のGemini APIキーを広告システムのデプロイに渡す(`-c geminiApiKey=…`)。
2. **広告インデックス再構築**: 既存の1024次元インデックスを削除し、3072次元で作り直す。
   ```
...
3. **プロバイダ切替デプロイ**:
   ...
   npx cdk deploy RAG-Ads-Api-Stack-dev RAG-Ads-Batch-Stack-dev --exclusively -c env=<env> \
     -c embedProvider=gemini -c embedDimension=3072 -c geminiApiKey=<KEY>
   ```
   ※ Api→Batch はレイヤー独立化済みのため個別 `--exclusively` で順に。
4. **全広告の再エンベド**: 配信中(delivering)広告のベクトルをGeminiで作り直す。
   管理API経由で各広告を一度 paused→再出稿→承認するか、再エンベド用バッチを実行(要実装。
   ロジックは admin-api の syncVector と同一)。
5. **θ_rel 再較正**: Titan向けの0.25はGeminiでは不適。数十件の代表質問で関連/無関連の類似度分布を計測し、
   `PUT /v1/params {"theta_rel": <新値>}` で調整(Titanでは関連0.32〜0.42/無関連<0.12 → 0.25 だった。
   Geminiでは分布が異なるため再測定必須)。
6. **媒体側**: 回答生成の Gemini 質問埋め込みを generate-ads の `questionVector` に渡す(再埋め込み省略)。
