 ...
 * サーバー側検証が正であり、クライアント検証は補助(SD-001 4.1節)。
 */
...
/**
 * @param {object} body 入力
 * @param {{draft?: boolean}} opts draft=trueは下書き保存(必須チェックを緩和し、入力済み項目の形式のみ検証)
  ...
  if (has(body.target)) {
    ...
    if (typeof t !== 'object' || Array.isArray(t)) push('target', 'ターゲット設定：形式が不正です');
      ...
      if (t.ageRange !== undefined && t.ageRange !== null) {
        const ok = Array.isArray(t.ageRange) && t.ageRange.length === 2
          && t.ageRange.every((n) => isInt(n) && n >= 18 && n <= 99) && t.ageRange[0] <= t.ageRange[1];
        if (!ok) push('target', 'ターゲット設定：年齢は18〜99の範囲で下限≦上限としてください');
      }
      if (t.questionTypes !== undefined && t.questionTypes !== null) {
        const ok = Array.isArray(t.questionTypes) && t.questionTypes.every((v) => QUESTION_TYPE_VALUES.includes(v));
        if (!ok) push('target', 'ターゲット設定：質問内容タイプの値が不正です');
      }
    }
  }
...
/** 入力からRagAds.META属性へ写像する(許可属性のみ。5.1節) */
